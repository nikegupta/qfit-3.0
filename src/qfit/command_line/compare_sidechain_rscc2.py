...
# Default minimum sidechain RMSD (Å) for two conformers of the same residue, across the
# separate refined-backbone PDB files for a dataset, to be treated as distinct. Below this, a
# conformer is considered a duplicate of one already collected and is not scored. Residues
# outside the refinement cutoff radius are typically identical across every file (since they
# were never moved by refinement), so this collapses to a single scored conformer for them;
# residues that were actually refined can have genuinely different coordinates per file (since
# each file's refinement was driven by a different ligand placement), so every distinct
# conformer among those is kept and scored.
DEFAULT_RMSD_CUTOFF = 0.1

# Residue names excluded when collecting unique residues from the refined-backbone files.
# LIG is excluded here because these are protein-backbone/sidechain RSCC comparisons; the
# ligand itself is scored elsewhere.
EXCLUDED_RESNAMES = {"LIG"}
...
def build_argparser():
    ...
    p.add_argument(
        'refined_pdb_glob',
        type=str,
        help="Template GLOB PATTERN matching each dataset's set of per-model refined-backbone PDB "
             "files (one file per model, NOT a multimodel PDB). Use '{dataset}' as a placeholder for "
             "the dataset name and '*' as a wildcard over model index, e.g. "
             "'/.../datasets/{dataset}/run_1/placer_100/filter_4/{dataset}_backbone_refined_*.pdb'"
    )
    p.add_argument(
        'output_folder',
        type=Path,
    ...
    p.add_argument(
        ...
        help="Minimum sidechain RMSD (Å) for two conformers of the same residue to be treated "
             "as distinct across the separate refined-backbone pdb files for a dataset",
    )
    return p


class RSCC_Comparator():
    def __init__(self, datasets_dir, reference_pdb_pattern, refined_pdb_glob_pattern, output_folder,
                 csv_file, n_cpus=24, rmsd_cutoff=DEFAULT_RMSD_CUTOFF):
        self.datasets_dir = datasets_dir
        self.reference_pdb_pattern = reference_pdb_pattern
        self.refined_pdb_glob_pattern = refined_pdb_glob_pattern
    ...
    def _get_unique_residues(self, models, exclude_resnames=None):
        """
        Walks a list of single-model Structures (one per refined-backbone PDB file for this
        dataset) and, for each (chain_id, resi), collects every conformer whose sidechain RMSD
        to all previously collected conformers of that residue exceeds self.rmsd_cutoff.
        Residues whose resname is in exclude_resnames (e.g. 'LIG') are skipped entirely.

        Residues outside the refinement cutoff radius are typically identical across every
        file (never moved by refinement), so this collapses to a single scored conformer per
        residue for them. Residues that were actually refined can have genuinely different
        coordinates per file (since each file's refinement was driven by a different ligand
        placement), so every distinct conformer among those is kept.
        ...
        """
        exclude_resnames = exclude_resnames or set()
    ...
    def _process_dataset(self, dataset, dataset_dir):
        ...
        refined_pdb_glob = self.refined_pdb_glob_pattern.format(dataset=dataset)

        if not reference_pdb.exists():
            print(f'Warning: no reference pdb found for {dataset} at {reference_pdb}, skipping.')
            return [], []

        refined_pdb_files = sorted(glob.glob(refined_pdb_glob))
        if not refined_pdb_files:
            print(f'Warning: no refined-backbone pdb files found for {dataset} matching '
                  f'{refined_pdb_glob}, skipping.')
            return [], []

        print(f'Processing {dataset}: resolution={resolution}, '
              f'{len(refined_pdb_files)} refined-backbone file(s) found')
        time0 = time.time()
        ...
        refined_models = [
            self._clean_structure(Structure.fromfile(str(refined_pdb_file)))
            for refined_pdb_file in refined_pdb_files
        ]
        refined_residues = self._get_unique_residues(refined_models, exclude_resnames=EXCLUDED_RESNAMES)

        shared_keys = sorted(set(reference_keys) & set(refined_residues.keys()))
        ...
        for key in shared_keys:
            ...
            test_resn, refined_residue, coor_set = refined_residues[key]

            if ref_resn != test_resn:
                print(f'WARNING: mismatched residue identity at {dataset} {chain_id}{resi}: '
                      f'reference={ref_resn}, refined={test_resn}. Skipping.')
            ...
            conformer_rsccs = self._score_conformers(
                refined_residue, coor_set, event_maps, event_maps_models, rmask
            )
    ...
    def _write_csv(self, results, mismatches):
        ...
        with open(rscc_output, 'w+') as f:
            f.write('dataset,residue,resname,reference_rscc,best_refined_rscc,median_refined_rscc,'
        ...
        if mismatches:
            ...
            with open(mismatch_output, 'w+') as f:
                f.write('dataset,residue,reference_resname,refined_resname')
                f.write('\n')
...
def main():
    ...
    comparator = RSCC_Comparator(
        args.datasets_dir, args.reference_pdb, args.refined_pdb_glob, args.output_folder,
