...
class _BaseEDIA:
                ...
                for ind in range(len(residue.name)):
                    ...
                    if target_residue != None:
                        ...
                        for idx in range(len(target_residue.name)):
                            if (
                                np.linalg.norm(coor - target_residue.coor[idx])
                                < 2.16 * 2 + 0.2
                            ):
                                flag = 1
                                break
    ...
    def ownership(self, p, dist, ed_radius, S, D, I):
        if dist / ed_radius >= 2.0:  # Grid point p is too far from the atom...
            ...
            if len(S) > 0:  # Another atom owns the grid point
                ...
                if (
                    len(D) == 1
                ):  # No other atom owns the grid point, target atom is the only atom in D.
                    o = 1.0
            ...
            for j in range(0, len(self.xmap.array[i])):
                for k in range(0, len(self.xmap.array[i][j])):
                    if self.xmap.array[i][j][k] - self.mean > contour * self.sigma:
                        coor = np.dot(
                            np.asarray([k, j, i]) + np.asarray(self.xmap.offset),
                            self.grid_to_cartesian,
                        )
                        print(
                            "HETATM {0:4d}  H   HOH A {0:3d}    {1:8.3f}{2:8.3f}{3:8.3f}  1.00 37.00           H".format(
                                1, coor[0], coor[1], coor[2]
                            )
                        )
    ...
    def covalently_bonded(self, atom_a, atom_b):
        error = (
            2 * 0.06
        )  # two standard deviations from the largest observed standard deviation for protein bond lengths
        try:
            if (
                np.linalg.norm(np.asarray(atom_a[0]) - np.asarray(atom_b[0]))
    ...
    def calculate_non_bonded(self, atom_a, set):
        ...
        for atom_b in set:
            if atom_a[4] == atom_b[4] and atom_a[3] != atom_b[3] and atom_a[3] < 1.0:
                continue
            if (
                not self.covalently_bonded(atom_a, atom_b)
                or np.linalg.norm(np.asarray(atom_a[0]) - np.asarray(atom_b[0])) < 0.01
            ):
                I.append(atom_b)
        return I
    ...
    def calc_edia(self, atom, element, charge, coor, occ, resi, bfactor):
        ...
        grid = np.dot(coor, self.cartesian_to_grid).astype(int) - np.asarray(
            self.xmap.offset
        )  # (x,y,z)
        # Look up the electron density radius on the lookup table:
        if charge == "":
            ed_radius = self.calculate_density_radius(
                element, self.options.resolution, bfactor
            )
        else:
            ed_radius = self.calculate_density_radius(
                element, self.options.resolution, bfactor, charge
            )
        ...
        return (
            ...
            sum_neg_product / sum_neg_weights,
            sum_product / sum_pos_weights,
        )
    ...
    def calc_edia_residue(self, residue):
        ...
        for ind in range(len(residue.name)):
            ...
            edia_plus[ind], edia_minus[ind], edia[ind] = self.calc_edia(
                atom, element, charge, coor, occ, residue.resi[ind], residue.b[ind]
            )
                ...
                if key != "":
                    ...
                    print(
                        "{0} {1} {2:.2f} {3:.2f} {4:.2f}".format(
                            residue.resi[0], key, occupancy[key], EDIAm, OPIA
                        )
                    )
        try:
            print(
                "{0} Comb {1:.2f} {2:.2f} {3:.2f}".format(
                    residue.resi[0],
                    sum(occupancy.values()) - occupancy[""],
                    EDIAm_Comb,
                    OPIA,
                )
            )
    ...
    def __call__(self):
        ...
        for chain in self.structure:
            ...
            for residue in chain:
                ...
                print(
                    "{0} {1:.2f} {2:.2f}".format(
                        residue.id[0], self.EDIAm[idx], self.OPIA[idx]
                    )
                )
                idx += 1
