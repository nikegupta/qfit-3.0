...
class QFitBindingSite(_BaseQFit):
    def __init__(self, conformer, structure, xmap, options):
        ...
        tl = 0
        for key in list(self.residues_in_binding_site.keys()):
            tl += len(self.residues_in_binding_site[key])
        print(tl)
    ...
    def _per_residue_solution(self):
        ...
        class QfitResidue(_BaseQFit):
            ...
            def _score_models_rscc(self, res):
                """Scores the best residue conformer using correlation coefficient against the target density"""
                best_score = np.inf
                best_model = np.nan

                if res.only_atom_group().resname.strip() == 'LIG':
                    print(f'writing output file for {res.resseq}, {res.only_atom_group().resname.strip()}')
                    with open('ligand_correlations.txt', 'w+') as f:
                        for i, model in enumerate(self._models):
                            corr = np.corrcoef(model, self._target)[0, 1]
                            score = 1 - corr
                            ...
                            if score < best_score:
                                best_score = score
                                best_model = i
                        f.write('best model:' + str(best_model) + ', Correlation: ' + str(best_score))
        ...
        for chain in model.chains():
            ...
            for res in chain.residue_groups():
                ...
                qfit_residue._bs = [qfit_residue._bs[best_coor_set]]
    ...
    def _naive_solution(self):
        ...
        with open('ligand_mses.txt','w+') as f:
            ...
            f.write('best model:' + str(best_model) + ', Mse: ' + str(best_mse))
        print(f'scored models in {time.time() - time0}')

        print(best_model)
        self._coor_set = [self._coor_set[best_model]]
        self._bs = [self._bs[best_model]]
        ...
        for j,model in enumerate(placer_models):
            ...
            for chain_id in list(self.residues_in_binding_site.keys()):
                for res_num in self.residues_in_binding_site[chain_id]:
                    ...
                    #add from placer model
                    for i in range(residue.coor.shape[0]):
                        placer_coor_set[(index + i),:] = residue.coor[i,:]
                        placer_b_set[(index + i)] = 20
                    index += residue.coor.shape[0]
