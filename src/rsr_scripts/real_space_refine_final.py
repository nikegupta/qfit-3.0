...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # catch that here (a bond stretched/collapsed by more than BOND_LENGTH_TOLERANCE) and
        # revert just that residue to its pre-refinement conformation, rather than letting a
        # chemically nonsensical result silently pass through to the output pdb.
        blown_out = find_blown_out_bonds(merged_array, refined_array, protein_residues)
        if blown_out:
            print(f"WARNING: {len(blown_out)} residue(s) had a bond length change of more than "
                  f"{BOND_LENGTH_TOLERANCE} A during refinement - reverting to their "
                  f"pre-refinement conformation:")
